...
**Project:** OpenFlow - AI-Powered Conversations  
...
### Production URL
**Live App:** https://openflow-cno6l2kfga-uc.a.run.app
...
```bash
gcloud beta run services add-iam-policy-binding openflow \
  --region=us-central1 \
  --member=allUsers \
...
gcloud run deploy openflow \
  ...
  --set-env-vars="GOOGLE_CLOUD_PROJECT=gen-lang-client-0986191192,NODE_ENV=production,VERTEX_AI_LOCATION=us-central1,PUBLIC_BASE_URL=https://openflow-cno6l2kfga-uc.a.run.app,BIGQUERY_DATASET=openflow_dataset,ENABLE_ANALYTICS=true,ENABLE_CHAT=true" \
  --project=gen-lang-client-0986191192
```
...
**Deployment Output:**
```
Service [openflow] revision [openflow-00004-7jg] has been deployed
Service URL: https://openflow-cno6l2kfga-uc.a.run.app
```
...
https://openflow-cno6l2kfga-uc.a.run.app     (add for production)
```

**Authorized redirect URIs:**
```
http://localhost:3000/auth/callback          (keep for local dev)
https://openflow-cno6l2kfga-uc.a.run.app/auth/callback  (add for production)
```
...
open https://openflow-cno6l2kfga-uc.a.run.app
...
curl -sI https://openflow-cno6l2kfga-uc.a.run.app/auth/login | grep Location
# Expected: Location: https://accounts.google.com/...

# View recent logs
gcloud logging read \
  "resource.type=cloud_run_revision AND resource.labels.service_name=openflow" \
  --limit=20 \
  --project=gen-lang-client-0986191192
...
gcloud builds submit \
  --tag gcr.io/gen-lang-client-0986191192/openflow \
  --project=gen-lang-client-0986191192
...
gcloud monitoring timeseries list \
  --filter='resource.type="cloud_run_revision" AND resource.label.service_name="openflow"' \
  --project=gen-lang-client-0986191192
...
gcloud run services update-traffic openflow \
  --to-revisions=openflow-00003-abc=100 \
  --region=us-central1 \
  --project=gen-lang-client-0986191192
```
...
### Production URLs
- **App:** https://openflow-cno6l2kfga-uc.a.run.app
- **OAuth Callback:** https://openflow-cno6l2kfga-uc.a.run.app/auth/callback

### GCP Console Links
- [Cloud Run Service](https://console.cloud.google.com/run/detail/us-central1/openflow?project=gen-lang-client-0986191192)
...
We successfully deployed OpenFlow from local development to production on Google Cloud Run, overcoming several technical challenges along the way:
