      ...
      >
        ...
        <div className="p-4 flex-1 grid grid-cols-2 gap-4 overflow-y-auto">
          ...
          <div className="space-y-3 flex flex-col">
            ...
            {/* Chunks Display Section - Separate section, always expanded */}
            {source.ragMetadata && source.ragEnabled && (
              <section className="bg-white rounded-lg border border-slate-300 p-3">
                ...
                {loadingChunks ? (
                  ...
                  <div className="space-y-2">
                    {/* Stats Summary */}
                    <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-2.5 grid grid-cols-3 gap-2 text-center">
                      <div>
                        <p className="text-[10px] text-slate-500 mb-0.5">Total</p>
                        <p className="text-base font-bold text-indigo-700">{chunks.length}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-slate-500 mb-0.5">Tokens</p>
                      ...
                      <div>
                        ...
                        <p className="text-base font-bold text-indigo-700">
                          {Math.round(chunks.reduce((sum, c) => sum + (c.metadata.tokenCount || 0), 0) / chunks.length)} tokens
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="max-h-96 overflow-y-auto space-y-2 pr-1">
                      {chunks.map((chunk, idx) => (
                        <button
                          ...
                          className="w-full text-left bg-white border border-slate-200 rounded-lg p-3 hover:border-indigo-400 hover:shadow-md transition-all group"
                        >
                          ...
                          <p className="text-xs text-slate-700 leading-relaxed line-clamp-3 group-hover:text-slate-900">
                            {chunk.text}
                          </p>
                          <div className="mt-2 pt-2 border-t border-slate-100 text-[10px] text-slate-400 flex items-center justify-between">
                            <span>{chunk.text.length.toLocaleString()} caracteres</span>
            ...
            {/* Original File Viewer - Below chunks section */}
