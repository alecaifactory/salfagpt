...
**Impact:**
- ❌ Console clutter with red error messages
- ❌ Unprofessional appearance in DevTools
- ❌ Potential performance impact (failed network requests)
- ✅ No visual/functional impact (Tailwind CSS loaded correctly via other means)
...
**Result:** ❌ False - Clearing cache didn't fix it

**Second Hypothesis:** Orphaned CSS files in `/public`  
**Result:** ✅ Partially correct - Files existed from old builds

**Third Hypothesis:** Stale build manifest  
**Result:** ✅ Correct - Manifest had hardcoded CSS references
...
vite: {
  build: {
    cssCodeSplit: false,  // ← Single CSS bundle instead of per-page
  }
}
```
...
<head>
  ...
  ${renderHead()}  ← Astro injects CSS here
</head>
```
...
<head>
  ...
  <link rel="stylesheet" href="/_tailwind-compiled.css">
  ${renderHead()}  ← Won't inject duplicate, our link loads first
</head>
```
...
**Before Fix:**
- ❌ Console: 4 CSS 404 errors
- ❌ JWT: "JWT_SECRET not configured" errors
- ❌ Auth: Not working

**After Fix:**
- ✅ CSS loads: HTTP 200 for `/_tailwind-compiled.css`
- ✅ JWT configured: Authentication working
- ✅ Console: Clean (no critical errors)
- ✅ Site functional: All features work
...
**DO:**
- ✅ Use `cssCodeSplit: false` for simpler CSS management
- ✅ Add manual `<link>` tags to pages
- ✅ Run `npm run build` locally before deploying
- ✅ Check dist/client/*.css to verify files generated
- ✅ Set all env vars in Cloud Run, not just in .env
- ✅ Delete unused pages/components promptly
- ✅ Add cache-control headers to prevent stale HTML

**DON'T:**
- ❌ Rely on Astro's automatic CSS injection
- ❌ Leave orphaned .astro pages in src/pages
- ❌ Deploy without verifying build locally
- ❌ Assume env vars from .env carry to Cloud Run
- ❌ Trust browser cache during debugging
...
4. **Browser Cache is Deceptive**
   - "It works in localhost" ≠ "It works in production"
