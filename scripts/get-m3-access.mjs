...
async function getM3Access() {
  try {
    console.log('🔍 Searching for GOP GPT M3...\n');
    ...
    accessList.forEach(user => {
      const emailPad = user.email.padEnd(32);
      const namePad = user.name.padEnd(40);
      const domainPad = user.domain.padEnd(20);
