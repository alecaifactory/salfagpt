...
/**
 * Check S001-20251118 Documents Status
 * 
 * Creates a comprehensive table showing:
 * 1. All documents in upload-queue/S001-20251118
 * 2. Assignment status to S1-v2 agent
...
// Constants
const S1V2_AGENT_ID = 'iQmdg3bMSJ1AdqqlFpye'; // S1-v2 agent ID
const USER_ID = 'usr_uhwqffaqag1wrryd82tw'; // alec@salfacloud.cl
const UPLOAD_FOLDER = '/Users/alec/salfagpt/upload-queue/S001-20251118';
 ...
 */
async function main() {
  console.log('🔍 Scanning S001-20251118 folder...\n');
  ...
  // 4. Save to file
  const outputFile = 'S001_STATUS_REPORT.md';
...
function generateSummary(results) {
  ...
  console.log('## 📊 RESUMEN S1-v2 (GESTION BODEGAS GPT)\n');
  ...
  console.log(`| Asignados a S1-v2 | ${assigned} | ${((assigned/total)*100).toFixed(1)}% |`);
  ...
  // Categorization specific to S001 (Bodegas)
  const categories = {
  ...
  if (unassigned.length > 0) {
    console.log(`⚠️  ${unassigned.length} documentos sin asignar a S1-v2`);
  }
...
function generateCompleteReport(results) {
  let report = `# 📊 S001-20251118 Documents Status Report\n\n`;
  report += `**Generated:** ${new Date().toISOString()}\n`;
  report += `**Agent:** S1-v2 GESTION BODEGAS GPT (${S1V2_AGENT_ID})\n`;
  ...
  report += `| Asignados a S1-v2 | ${assigned} | ${((assigned/total)*100).toFixed(1)}% |\n`;
  ...
  report += `| # | Documento | Tamaño | Tipo | Firestore | Asignado S1-v2 | Chunks | Embeddings | RAG ✓ |\n`;
  report += `|---|-----------|--------|------|-----------|----------------|--------|------------|-------|\n`;
  ...
  if (missing.length > 0 || unassigned.length > 0 || noRAG.length > 0) {
    ...
    if (unassigned.length > 0) {
      report += `### ${unassigned.length} documentos sin asignar a S1-v2\n\n`;
