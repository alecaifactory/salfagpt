/**
 * Test Real Cartola - Simple ES Module Test
 */
...
async function testRealCartola() {
  try {
    ...
    const module = await import('../dist/chunks/nubox-cartola-extraction_CZV2sNOE.mjs').catch(async () => {
      // Try alternative paths
      return await import('../src/lib/nubox-cartola-extraction.js');
    });
    
    const extractNuboxCartola = module.extractNuboxCartola;
    
    if (!extractNuboxCartola) {
      throw new Error('extractNuboxCartola function not found in module');
    }
    ...
    result.movements.slice(0, 3).forEach((mov, idx) => {
      ...
      if (mov.sender_account) {
        ...
        console.log(`     - dv:          "${mov.sender_account.dv || 'N/A'}"`);
    ...
    // Show sample JSON
    console.log('📄 SAMPLE MOVEMENT JSON (First Movement):');
